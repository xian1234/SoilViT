...
class GeoDataset(Dataset):
    """
    Loads patch pairs from a JSON config produced by data generation / spatial_data_split.py.
    ...
    def __getitem__(self, idx: int):
        ...
        for band in range(input_data.shape[0]):
            input_data[band] = (input_data[band] - self.input_means[band]) \
                               / (self.input_stds[band] + 1e-8)
